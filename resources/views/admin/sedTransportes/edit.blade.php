...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sedTransporte.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sed-transportes.update", [$sedTransporte->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('zona') ? 'is-invalid' : '' }}" name="zona" id="zona">
                    ...
                    @foreach(App\Models\SedTransporte::ZONA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('zona', $sedTransporte->zona) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('zona'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('comuna') ? 'is-invalid' : '' }}" name="comuna_id" id="comuna_id">
                    @foreach($comunas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('comuna_id') ? old('comuna_id') : $sedTransporte->comuna->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('institucion') ? 'is-invalid' : '' }}" name="institucion_id" id="institucion_id">
                    @foreach($institucions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('institucion_id') ? old('institucion_id') : $sedTransporte->institucion->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sede') ? 'is-invalid' : '' }}" name="sede_id" id="sede_id">
                    @foreach($sedes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sede_id') ? old('sede_id') : $sedTransporte->sede->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('sede'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('grado') ? 'is-invalid' : '' }}" type="text" name="grado" id="grado" value="{{ old('grado', $sedTransporte->grado) }}">
                @if($errors->has('grado'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('beneficiarios') ? 'is-invalid' : '' }}" type="number" name="beneficiarios" id="beneficiarios" value="{{ old('beneficiarios', $sedTransporte->beneficiarios) }}" step="1">
