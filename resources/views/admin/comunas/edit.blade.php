...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.comuna.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.comunas.update", [$comuna->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.comunas.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $comuna->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
