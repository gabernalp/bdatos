...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sedEstimulosGestore.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sed-estimulos-gestores.update", [$sedEstimulosGestore->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nombre">{{ trans('cruds.sedEstimulosGestore.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $sedEstimulosGestore->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('linea_participacion') ? 'is-invalid' : '' }}" type="text" name="linea_participacion" id="linea_participacion" value="{{ old('linea_participacion', $sedEstimulosGestore->linea_participacion) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('proyecto') ? 'is-invalid' : '' }}" type="text" name="proyecto" id="proyecto" value="{{ old('proyecto', $sedEstimulosGestore->proyecto) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado" id="estado" required>
                    ...
                    @foreach(App\Models\SedEstimulosGestore::ESTADO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('estado', $sedEstimulosGestore->estado) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_presentacion') ? 'is-invalid' : '' }}" type="text" name="fecha_presentacion" id="fecha_presentacion" value="{{ old('fecha_presentacion', $sedEstimulosGestore->fecha_presentacion) }}" required>
